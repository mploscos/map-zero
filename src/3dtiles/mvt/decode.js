// Derived from CesiumJS 1.145.0 / @cesium/engine 26.3.0.
// Copyright 2011-2024 CesiumJS Contributors. SPDX-License-Identifier: Apache-2.0
// Modifications: use native Error instead of Cesium RuntimeError.
// See NOTICE.md and LICENSE.md in this directory.
// @ts-check
...
function readVarint32(bytes, pos, limit) {
  ...
  if (value.value > 0xffffffffn) {
    throw new Error("Invalid MVT uint32 varint.");
  }
  return {
...
function readVarintLength(bytes, pos, limit) {
  ...
  if (value.value > BigInt(Number.MAX_SAFE_INTEGER)) {
    throw new Error("Invalid MVT length varint.");
  }
  return {
    value: Number(value.value),
    newPos: value.newPos,
...
function readBigVarint(bytes, pos, limit, maxBytes) {
  ...
  while (true) {
    if (pos >= limit || pos >= bytes.length) {
      throw new Error("Invalid MVT: truncated varint.");
    }
    ...
    if (byteCount >= byteLimit) {
      throw new Error("Invalid MVT: varint is too long.");
    }
    shift += 7n;
  }
...
function advanceByLength(pos, length, limit, fieldName) {
  if (!Number.isFinite(length) || length < 0) {
    throw new Error(`Invalid MVT ${fieldName}: invalid length.`);
  }
