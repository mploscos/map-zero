 ...
 * Layer opacity is applied separately, so the controller still fades all labels.
 * @param {{text:string, priority:number, font:string, fill:string, halo:string, haloWidth:number}} descriptor
 * @param {{fontScale?:number, opacity?:number, haloOpacity?:number, haloWidth?:number}} [style]
 */
export function cesiumLabelPresentation(descriptor, style = {}) {
  const fontScale = style.fontScale ?? 1.3;
  const fill = Color.fromCssColorString(descriptor.fill);
  const halo = Color.fromCssColorString(descriptor.halo);
  // Preserve explicitly transparent theme colors.
  if (fill.alpha > 0) fill.alpha = style.opacity ?? 1;
  if (halo.alpha > 0) halo.alpha = style.haloOpacity ?? 1;
  return {
    ...descriptor,
    font: descriptor.font.replace(/([\d.]+)px/, (_, size) => `${Math.round(Number(size) * fontScale)}px`),
    fill: fill.toCssColorString(),
    halo: halo.toCssColorString(),
    haloWidth: style.haloWidth ?? Math.max(4, descriptor.haloWidth)
  };
}
...
/** Labels use batch metadata in visible static 3D content; no separate transport. */
export function createCesiumLabels(viewer, tilesets, options) {
  ...
  function candidatesFor(content) {
    ...
    const candidates = [];
    for (const part of content.innerContents ?? [content]) {
      for (let i = 0; i < part.featuresLength; i++) {
        const feature = part.getFeature(i, 0);
        if (!feature) continue;
        ...
        const source = String(feature.getProperty('mapzero_layer') ?? feature.getProperty('layer') ?? '');
        const layer = source;
        ...
        candidates.push({ key: `${layer}:${id}`, layer, adapter, position: Cartesian3.fromDegrees(lon, lat, 15) });
      }
    }
  ...
  function update() {
    ...
    const candidates = [];
    const zoom = Math.round(options.getZoom()*4)/4;
    ...
    for (const content of visible) {
      for (const candidate of candidatesFor(content)) {
        ...
        const manifestLayer = layers.get(layer) ?? layers.get(layer === 'aip' ? 'aviation' : layer);
        if (manifestLayer && !isLayerInZoomRange(manifestLayer, zoom)) continue;
        if (zoom < (adapter.get('mapzero_minzoom') ?? 0) || zoom > (adapter.get('mapzero_maxzoom') ?? 24)) continue;
        ...
        if (candidate.zoom !== zoom) {
          ...
          if (candidate.descriptor) {
            candidate.descriptor = cesiumLabelPresentation(candidate.descriptor, options.styleDocument.labels?.cesium);
          }
    ...
    for (const candidate of selected) {
      ...
      const properties = {
        ...
        heightReference: options.clampToTerrain ? HeightReference.CLAMP_TO_TERRAIN : HeightReference.NONE, disableDepthTestDistance: Number.POSITIVE_INFINITY
      };
  ...
  const removers = [
    ...
    ...tilesets.map(tileset => tileset.tileVisible.addEventListener((tile) => visible.add(tile.content))),
    ...tilesets.map(tileset => tileset.tileUnload.addEventListener((tile) => { visible.delete(tile.content); cache.delete(tile.content); })),
